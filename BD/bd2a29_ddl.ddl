...
CREATE OR REPLACE TRIGGER FKNTM_Moduly_w_obszarze BEFORE
  UPDATE OF ID_obszaru,ID_modulu ON Moduly_w_obszarze 
FOR EACH ROW    
WHEN (new.ID_obszaru <> old.ID_obszaru or new.ID_modulu <> old.ID_modulu)
    BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table Moduly_w_obszarze is violated');
END;
/

CREATE OR REPLACE TRIGGER FKNTM_Pracochlonnosci BEFORE
  UPDATE OF ID_obszaru,ID_Oferty ON Pracochlonnosci 
FOR EACH ROW    
WHEN (new.ID_obszaru <> old.ID_obszaru or new.ID_Oferty <> old.ID_Oferty)
    BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table Pracochlonnosci is violated');
END;
/
